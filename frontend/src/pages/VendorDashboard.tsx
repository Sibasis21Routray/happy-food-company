...
import { Package, CheckCircle, DollarSign, Calendar, Clock, Search } from 'lucide-react';
    ...
    try {
      ...
      if (activeTab === 'analytics') {
        ...
        const qParams = new URLSearchParams({ 
          ...
          status: isRev ? 'all' : assignmentState.status, 
      ...
      <main className="container mx-auto px-4 mt-6 md:mt-8">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-200 mb-8 gap-6">
          <div>
            ...
            <p className="text-gray-500 font-bold text-xs md:text-sm mt-1">Manage operations and performance in real-time.</p>
          </div>
        ...
        {activeTab === 'analytics' ? (
              ...
              <button onClick={() => setActiveTab('revenue')} className="text-left w-full transition-transform active:scale-95 group">
                <div className="bg-gradient-to-br from-[#FA6011] to-[#ff3c83] p-8 rounded-[2rem] shadow-lg shadow-orange-100/50 text-white relative overflow-hidden h-full">
                  <div className="relative z-10">
                    ...
                    <p className="text-[11px] font-black text-white/70 uppercase tracking-widest mb-1">Total Revenue</p>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom duration-500">
              ...
              <div className="flex flex-col xl:flex-row justify-between items-stretch xl:items-center gap-8">
                <div className="flex-1">
                  <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-6 flex items-center gap-2">
                    Financial Ledger
                    <span className="bg-green-50 text-green-600 text-[10px] px-3 py-1 rounded-full uppercase tracking-widest font-black border border-green-100/50">Settled</span>
                  </h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Filter Ledger</p>
                ...
                <div className="bg-slate-900 p-8 rounded-[2rem] text-white w-full xl:max-w-xs relative overflow-hidden shadow-2xl flex flex-col justify-center">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 relative z-10">Period Earnings</p>
                  ...
                  <div className="mt-4 flex items-center gap-3 relative z-10">
                    <div className="bg-white/10 px-3 py-1.5 rounded-xl border border-white/5">
                      <p className="text-[9px] font-black uppercase text-slate-400">Total Orders</p>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden relative min-h-[400px]">
              {loading && <div className="absolute inset-0 bg-white/60 z-10 flex flex-col items-center justify-center"><div className="w-10 h-10 border-4 border-orange-100 border-t-[#FA6011] rounded-full animate-spin mb-4" /><p className="text-[10px] font-black text-[#FA6011] uppercase tracking-widest">Searching Ledger...</p></div>}
                ...
                <table className="w-full text-left min-w-[800px]">
                  ...
                  <tbody className="divide-y divide-gray-50 font-bold text-gray-600">
                    {revenueState.data.length > 0 ? revenueState.data.map((o: any) => (
                      ...
                      <tr><td colSpan={5} className="py-20 text-center font-black text-gray-300 text-xs uppercase tracking-widest">No matching transactions</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              <div className="p-6 border-t border-gray-100 bg-gray-50/30 flex flex-col sm:flex-row justify-between items-center gap-4">
                <span className="text-[10px] font-black text-gray-400">Page {revenueState.page} of {revenueState.totalPages}</span>
                <div className="flex gap-2 w-full sm:w-auto">
                  <button disabled={revenueState.page === 1} onClick={() => setRevenueState(prev => ({ ...prev, page: prev.page - 1 }))} className="flex-1 sm:flex-none px-6 py-2 bg-white border border-gray-200 rounded-xl text-[10px] font-black uppercase tracking-widest">Prev</button>
                  <button disabled={revenueState.page === revenueState.totalPages} onClick={() => setRevenueState(prev => ({ ...prev, page: prev.page + 1 }))} className="flex-1 sm:flex-none px-6 py-2 bg-white border border-gray-200 rounded-xl text-[10px] font-black uppercase tracking-widest">Next</button>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex-1 w-full">
                  <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-4 flex items-center gap-2">
                    Active Assignments
                    <span className="bg-orange-50 text-[#FA6011] text-[10px] px-3 py-1 rounded-full uppercase tracking-widest font-black border border-orange-100/50 ml-2">{assignmentState.totalOrders} items</span>
                  </h2>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden relative">
              <div className="overflow-x-auto text-[11px]">
                <table className="w-full text-left min-w-[1000px]">
                  <thead className="bg-[#fcfcfc] border-b border-gray-100">
                    <tr>{['Order Info', 'Customer', 'Items Ordered', 'Amount', 'Status Tracking', 'Actions'].map((h) => (<th key={h} className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">{h}</th>))}</tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50 font-bold text-gray-600">
                    {assignmentState.data.map((o: any) => (
                      <tr key={o._id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-8 py-4 text-slate-800">#{o._id.slice(-6).toUpperCase()}</td>
                        <td className="px-8 py-4 font-bold text-gray-500">{o.userId?.fullName || 'Guest'}</td>
                        <td className="px-8 py-4"><div className="flex flex-col gap-1 max-w-[200px]">{o.items?.map((item: any, i: number) => (<div key={i} className="flex justify-between items-center text-[10px] text-gray-600">{item.title} <span className="text-[#FA6011] ml-2">×{item.quantity}</span></div>))}</div></td>
                        <td className="px-8 py-4 font-black">₹{o.totalAmount}</td>
                        <td className="px-8 py-4">
                          <select value={o.status} onChange={(e) => updateOrderStatus(o._id, e.target.value)} className="w-full px-3 py-1.5 rounded-lg border-orange-100 bg-orange-50/30 text-[#FA6011] text-[10px] font-black uppercase outline-none transition-all">{['pending', 'confirmed', 'shipped', 'delivered', 'cancelled'].map(s => (<option key={s} value={s}>{s}</option>))}</select>
                        </td>
                        <td className="px-8 py-4"><button onClick={() => navigate(`/orders/${o._id}`)} className="text-[#FA6011] font-black uppercase text-[10px] hover:underline">Track</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="p-6 border-t border-gray-100 bg-gray-50/30 flex flex-col sm:flex-row justify-between items-center gap-4">
                <span className="text-[10px] font-black text-gray-400">Page {assignmentState.page} of {assignmentState.totalPages}</span>
                <div className="flex gap-2 w-full sm:w-auto">
                  <button disabled={assignmentState.page === 1} onClick={() => setAssignmentState(prev => ({ ...prev, page: prev.page - 1 }))} className="flex-1 sm:flex-none px-6 py-2 bg-white border border-gray-200 rounded-xl text-[10px] font-black uppercase tracking-widest">Prev</button>
                  <button disabled={assignmentState.page === assignmentState.totalPages} onClick={() => setAssignmentState(prev => ({ ...prev, page: prev.page + 1 }))} className="flex-1 sm:flex-none px-6 py-2 bg-white border border-gray-200 rounded-xl text-[10px] font-black uppercase tracking-widest">Next</button>
                </div>
              </div>
            </div>
          </div>
